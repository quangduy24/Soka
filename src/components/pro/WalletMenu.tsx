...
import { fetchWalletTokens, fmt, useHiddenPref } from './walletUtils.js';
...
function Avatar({ sym }: { sym: string }) {
  ...
  return (
    <span
      className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-white/40 font-mono text-[8px] font-bold text-white"
...
export const WalletMenu: React.FC<WalletMenuProps> = ({ walletAddress, onDisconnect }) => {
  ...
  return (
    <div className="relative" ref={rootRef}>
      <button
        onClick={() => setOpen((v) => !v)}
        className={`flex items-center gap-2 rounded-xl border border-pink-200/90 px-3.5 py-1.5 font-mono text-xs text-slate-800 font-bold backdrop-blur-sm shadow-2xs transition-all ${
          open ? 'bg-pink-100/90 border-pink-300' : 'bg-pink-50/80 hover:bg-pink-100/80'
        }`}
        title="Wallet balances on Mezo Testnet"
      >
        <span className="dot dot-accent animate-pulse" />
        <span>{addr}</span>
        <ChevronDown
          className={`h-3.5 w-3.5 transition-transform text-pink-600 ${open ? 'rotate-180' : ''}`}
        />
      </button>
          ...
          <div className="absolute right-0 top-[calc(100%+8px)] z-[70] flex max-h-[min(70vh,480px)] w-[320px] flex-col overflow-hidden rounded-2xl border border-white/50 bg-white/60 shadow-[0_20px_60px_-16px_rgba(0,0,0,0.2),0_8px_24px_-8px_rgba(0,0,0,0.12),inset_0_1px_0_rgba(255,255,255,0.8)] backdrop-blur-xl pop-in">
            ...
            <div className="flex shrink-0 items-center justify-between gap-2 border-b border-white/20 bg-white/20 px-3 py-2">
              <div className="flex items-center gap-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.14em] text-[#0f172a]/60">
                <Wallet className="h-3 w-3" /> Mezo Balances
              </div>
              <div className="flex items-center gap-1">
                <button
                  onClick={toggleHidden}
                  title={hidden ? 'Show balances' : 'Hide balances'}
                  className="flex h-6 w-6 items-center justify-center rounded-full border border-white/30 bg-white/20 text-[#0f172a]/60 transition-colors hover:bg-white/40"
                >
            ...
            <div className="flex shrink-0 flex-col gap-2 border-b border-[#F05391]/20 bg-[#F8E8EE] px-4 py-4">
              <span
                className="flex items-center gap-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.2em] text-[#F05391]"
              ...
              <span className="flex items-baseline justify-between gap-2">
                <span
                  className="text-[#1a1a2e] tracking-tight"
                  style={{
              ...
              {isError ? (
                ...
                <div className="flex flex-col gap-0.5">
                  {balances.map((t) => (
                    <div
                      key={t.coinType}
                      className="flex items-center gap-2 rounded-xl border border-transparent px-1.5 py-1.5 transition-colors hover:border-[#F05391]/20 hover:bg-white/20"
                    >
                      <Avatar sym={t.sym} />
                      <div className="min-w-0 flex-1 leading-none">
                        <div
                          className="truncate font-mono text-[11px] font-bold text-[#0f172a]"
                          style={{ fontFamily: 'var(--font-body)' }}
                        >
                          {t.sym}
                        </div>
                        <div className="mt-0.5 truncate font-mono text-[7.5px] text-[#0f172a]/40">
                          {t.coinType.length > 16 ? `${t.coinType.slice(0, 6)}...${t.coinType.slice(-4)}` : t.coinType}
                        </div>
                      </div>
            ...
            <button
              ...
              className="flex shrink-0 items-center justify-center gap-1.5 border-t border-white/20 bg-white/10 py-2 font-mono text-[10px] font-bold text-[#ef4444] transition-colors hover:bg-[#ef4444]/10 cursor-pointer"
            >
